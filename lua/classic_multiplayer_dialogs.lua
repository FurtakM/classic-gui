...
function createClassicLobby(PARENT, AFTER_OK, AFTER_CANCEL)
	...
	local ELEMENT = getElementEX(PARENT,anchorNone,XYWH(0,0,452,195), --X,Y matters not, @see ShowDialog
                                     false,{texture = 'classic/edit/background_createserver.png' ,}); --do not create as visible. The use casee is to create at init but show only after a while.
	...
	ELEMENT.CANCEL = clButton(ELEMENT, ELEMENT.width/2 +8,ELEMENT.height-30-17,150,30 , loc(TID_msg_Cancel), hideSelf .. AFTER_CANCEL,{} );
	
	ELEMENT.OK = clButton(ELEMENT, ELEMENT.width/2 - 159,ELEMENT.height-30-17,150,30, loc(TID_msg_Ok), hideSelf .. AFTER_OK , {}); 

	--used to use ADMUI3L   Not supported all characters
	ELEMENT.SERVER_Label = getLabelEX(ELEMENT, anchorTLR,XYWH((ELEMENT.width-250)/2-2,20,30,250,20),BankGotic_14, loc(TID_MultiServerNameLBL), 
    {
	...
	ELEMENT.SERVER = getEditEX(ELEMENT, anchorTLR, XYWH((ELEMENT.width-249)/2,35,249,20),BankGotic_14,loc_format(TID_Lobby_AutoServerName,{ getvalue(OWV_USERNAME)}),'', {},{
		font_colour = BLACKA(255);
		colour = WHITEA(255),
	}  );
	
	ELEMENT.PASSWD_Label = getLabelEX(ELEMENT, anchorTLR,XYWH((ELEMENT.width-250)/2-2,90,30,250,20),BankGotic_14, loc(TID_MultiEnterARoomPass), {
		font_colour = BLACKA(255);
        shadowtext = false,
        nomouseevent = true,
        text_halign = ALIGN_LEFT,
	...
	ELEMENT.PASSWD = getEditEX(ELEMENT, anchorTLR, XYWH((ELEMENT.width-249)/2,106,249,19),BankGotic_14,'','', {},{
		font_colour = BLACKA(255);
		colour = WHITEA(255),
		disabled = true;
		visible = true;
	});
	ELEMENT.PASSWD_BACK = getElementEX(ELEMENT, anchorTLRB, XYWH(ELEMENT.PASSWD.x,ELEMENT.PASSWD.y,ELEMENT.PASSWD.width, ELEMENT.PASSWD.height),true, {
	...
	ELEMENT.USE_PASSWD = clCheckbox(ELEMENT,(ELEMENT.width-250)/2,69,'eReg:get('..ELEMENT.PASSWD.ID.. '):switch()', {checked = not ELEMENT.PASSWD.disabled });
	ELEMENT.USE_PASSWD_LABEL = getLabelEX(ELEMENT, anchorTLR,XYWH((ELEMENT.width-250)/2+20,69,150,20),BankGotic_14, "USE PASSWORD", --LACKS LOCALIZATION 
    {
		font_colour = BLACKA(255);
        shadowtext = false,
        nomouseevent = true,
